...
function clicktrends_update_elementor($request) {
    ...
    if (is_string($elementor_data)) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($data_array)) {
            ...
            $new_elementor_data = str_ireplace($search_text, $replace_text, $elementor_data, $count);
...
function clicktrends_normalize_string($str) {
    // Decode HTML entities (like &nbsp; or &amp;)
    $decoded = html_entity_decode($str, ENT_QUOTES | ENT_HTML5, 'UTF-8');
    // Strip tags
    $stripped = strip_tags($decoded);
    // Replace all whitespace (including weird unicode spaces) with a single space
    $normalized = preg_replace('/[\s\x{00A0}]+/u', ' ', trim($stripped));
    return $normalized;
}

function clicktrends_recursive_replace($search, $replace, $array, &$replaced) {
    $normalized_search = clicktrends_normalize_string($search);
    ...
    foreach ($array as $key => &$value) {
        if (is_array($value)) {
            ...
            if (stripos($value, $search) !== false) {
                ...
                $normalized_val = clicktrends_normalize_string($value);
                
                if (stripos($normalized_val, $normalized_search) !== false) {
                    if (strlen($normalized_val) < strlen($normalized_search) + 100) {
